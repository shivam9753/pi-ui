...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-wrap gap-3 items-center mb-4">
      ...
      <div *ngFor="let config of filterConfigs" class="relative">
        
        <!-- Select Filter -->
        <div *ngIf="config.type === 'select'" class="relative">
          <div class="flex items-center space-x-2 px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm hover:border-gray-400 focus-within:border-orange-500 focus-within:ring-1 focus-within:ring-orange-500 transition-all duration-150">
            <span class="text-sm font-medium text-gray-700 whitespace-nowrap">{{ config.label }}</span>
            <select [(ngModel)]="filters[config.key]" 
                    (ngModelChange)="onFilterChange()"
                    class="bg-transparent border-none outline-none text-sm text-gray-900 cursor-pointer pr-6 appearance-none">
        ...
        <!-- Search Filter -->
        <div *ngIf="config.type === 'search'" class="relative min-w-[200px]">
          <div class="flex items-center space-x-2 px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm hover:border-gray-400 focus-within:border-orange-500 focus-within:ring-1 focus-within:ring-orange-500 transition-all duration-150">
            ...
            <input type="text" 
                   ...
                   class="bg-transparent border-none outline-none text-sm text-gray-900 placeholder-gray-500 flex-1">
          </div>
        </div>

        <!-- Date Range Filter -->
        <div *ngIf="config.type === 'date-range'" class="flex items-center space-x-2">
          <div class="flex items-center space-x-2 px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm hover:border-gray-400 focus-within:border-orange-500 focus-within:ring-1 focus-within:ring-orange-500 transition-all duration-150">
            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Quick Filters -->
    <div *ngIf="quickFilters && quickFilters.length > 0" class="mb-4">
      <div class="text-sm font-medium text-gray-700 mb-2">Quick Filters:</div>
      <div class="flex flex-wrap gap-2">
        <button *ngFor="let quickFilter of quickFilters"
