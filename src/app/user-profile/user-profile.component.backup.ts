...
export class UserProfileComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      if (routeUserId) {
        // Specific user profile requested
        this.loadUserProfile(routeUserId);
      } else {
        // No user ID - load current user's own profile
      ...
      this.backendService.getUserSubmissions().subscribe({
        next: (response: any) => {
          ...
          if (response.submissions && response.submissions.length > 0) {
            console.log('🎯 First submission sample:', response.submissions[0]);
          }
          
          this.submissions.set(response.submissions || []);
          console.log('💾 Submissions set in component:', this.submissions());
  ...
  getAllSubmissionsChronological() {
    ...
    console.log('🔍 getAllSubmissionsChronological called - submissions:', submissions);
    console.log('📊 Submissions length:', submissions.length);
    
    const sorted = submissions
      .sort((a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime());
    
    console.log('📋 Sorted submissions:', sorted);
    return sorted;
  }
