...
import { ConsistentSubmissionMobileCardComponent, createPendingReviewActions, DataTableComponent, PaginationConfig, PENDING_REVIEWS_TABLE_COLUMNS, SimpleFilterOptions, SimpleSubmissionFilterComponent, SUBMISSION_BADGE_CONFIG, SubmissionAction, TableAction, TableColumn } from '../../shared/components';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, DataTableComponent, AdminPageHeaderComponent, ConsistentSubmissionMobileCardComponent, SimpleSubmissionFilterComponent],
...
export class ShortlistedSubmissionsComponent implements OnInit {
  ...
  loadShortlistedSubmissions() {
    ...
    this.backendService.getSubmissions({
      ...params,
      status: 'shortlisted' // Shortlisted submissions have this specific status
    }).subscribe({
      next: (response: any) => {
