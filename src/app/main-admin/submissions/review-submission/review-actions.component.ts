    ...
    @if (canReview && !showReviewForm) {
      <div class="flex flex-col sm:flex-row sm:justify-center gap-3">
        ...
        <button
          ...
          [disabled]="isSubmitting"
          class="w-full sm:w-auto px-6 py-3 text-sm font-semibold text-amber-800 bg-amber-100 rounded-lg hover:bg-amber-100 transition-colors duration-200 disabled:opacity-50 flex items-center gap-2 shadow-sm hover:shadow-md">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export class ReviewActionsComponent {
  ...
  getActionIconClass(): string {
    switch (this.reviewAction) {
      ...
      case 'revision': return 'bg-amber-100';
